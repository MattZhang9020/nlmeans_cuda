...
#define PATCH_SIZE 5
#define SHARED_SIZE (BZ + 2 * (SEARCH_WINDOW / 2 + PATCH_SIZE / 2))
...
__global__ void nlmeans_kernel(const float* d_noisy, float* d_denoised, int width, int height, int channels, int patch_radius, int search_radius) {
    ...
    for (int wy = -search_radius; wy <= search_radius; ++wy) {
        for (int wx = -search_radius; wx <= search_radius; ++wx) {
            ...
            int global_ny = blockIdx.y * blockDim.y + ny;
...
void nlmeans_cuda(const vector<float>& noisy, vector<float>& denoised) {
    ...
    dim3 block(BZ, BZ);
    dim3 grid((width + BZ - 1) / BZ, (height + BZ - 1) / BZ);
